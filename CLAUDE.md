...
- **コードを変更したら、その都度 GitHub にプッシュする。** ローカルにコミットを溜めない。
- 作業単位は「1つの意味のある変更 = 1コミット」。無関係な変更を1つのコミットに混ぜない。
- ブランチは `main` のみ。小規模なプロジェクトのため feature ブランチは切らない。
...
- **日本語で書く。** 1行目は50文字程度の要約、必要なら空行を挟んで本文で詳細を書く。
- 「修正」「更新」だけで終わらせず、何をどう変えたかがわかる粒度で書く。

### 守ること

- プッシュ前に `git status` で意図しないファイルが含まれていないか確認する。
- `.env` や APIキーなど機密情報は絶対にコミットしない（`.gitignore` で除外済み）。
- `git config` を変更する場合は **必ず `--local` を使う。`--global` は書き換えない**（他プロジェクトの署名に影響するため）。
- `git push --force` は使わない。履歴を書き換えたい場合は事前に相談する。
- プッシュ後は反映されたことを確認し、コミットハッシュを報告する。
